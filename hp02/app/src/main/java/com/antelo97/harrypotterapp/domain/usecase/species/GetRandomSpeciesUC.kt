...
class GetRandomSpeciesUC @Inject constructor(private val repository: SpeciesRepository) {

    suspend operator fun invoke(): List<Species> {
        val randomSpecies: Species = repository.getSpeciesFromDatabase().random()
        return listOf(randomSpecies)
    }
}
...
class SpeciesViewHolder(
    ...
    fun render(species: Species, position: Int) {
        itemSpeciesBinding.tvSpeciesName.text = species.name
        itemSpeciesBinding.tvSpeciesDetails.text = species.native_
        Picasso.get().load(species.imageUrl).into(itemSpeciesBinding.ivSpecies)

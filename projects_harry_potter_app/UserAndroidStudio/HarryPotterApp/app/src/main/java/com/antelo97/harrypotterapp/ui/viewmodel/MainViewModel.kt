...
import DeleteFavoriteBookUC
import GetFavoriteBooksUC
import InsertFavoriteBookUC
...
class MainViewModel @Inject constructor(
    ...
    private val insertUserUC: InsertUserUC,
    private val insertFavBookUC: InsertFavoriteBookUC,
    private val deleteFavBookUC: DeleteFavoriteBookUC
) : ViewModel() {

    // User
    val favoriteBooks: MutableLiveData<List<Book>> = MutableLiveData<List<Book>>()
    val favoriteCharacters: MutableLiveData<List<Book>> = MutableLiveData<List<Book>>()
    val favoriteSpells: MutableLiveData<List<Book>> = MutableLiveData<List<Book>>()
    val favoriteSpecies: MutableLiveData<List<Book>> = MutableLiveData<List<Book>>()
    ...
    val tvEmptyList: MutableLiveDablota<Boolean> = MutableLiveData<Boolean>()
    ...
    private suspend fun loadBooks() {
        ...
        favoriteBooks.postValue(getFavoriteBooksUC())
    }
    ...
    suspend fun insertUser(user: FirebaseUser): User {
        return insertUserUC(user)
    }

    suspend fun insertOrDeleteFavoriteBook(book: Book){
        insertFavBookUC(book.getIdApiBook())
        insertFavBookUC(book.getIdApiBook())
        favoriteBooks.postValue(getFavoriteBooksUC())

        deleteFavBookUC(book.getIdApiBook())
        favoriteBooks.postValue(getFavoriteBooksUC())
    }
}